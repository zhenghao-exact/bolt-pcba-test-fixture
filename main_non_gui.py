"""Headless Bolt PCBA test fixture runner with a per-test run/skip gate.

This is the no-GUI counterpart of bolt_fixture_main.py. It carries the full
production logic — BLE retry + first-failure handling, PPK2-lost / abnormal
reading escalation with persistent counters, --SG and --SKIP_CAL modes,
best-effort label printing, CSV write + Google Drive upload, persistent stdout
tee, and a board-after-board while-loop — but drives all operator interaction
through the terminal (print/input) instead of Tk windows.

The one behavioral addition over bolt_fixture_main.py: before every test step
the operator is prompted to RUN or SKIP that step. Skipping records the step as
skipped (truthy, so it does not fail the overall result) and continues to the
next step.

Usage:
    python3 main_non_gui.py            # full interactive flow
    python3 main_non_gui.py --SKIP_CAL # skip analog calibration (marked green)
    python3 main_non_gui.py --SG       # strain-gauge mode (skip cal/IMU/sleep)
"""
...
class ConsoleUI:
    """Terminal stand-in for gui.App.

    Exposes the same method names and return values that bolt_fixture_main.py's
    run_bolt_test()/main() call on the GUI App, so the ported flow stays nearly
    identical. Every prompt blocks on input() internally (the GUI used a
    wait_variable dance in the caller; here the blocking lives in the method).
    """
    ...
    def _expand_short_bolt_id(raw: str) -> str:
        """Expand short numeric input (e.g. '181') to 'Bolt_30000181'.

        Mirrors the nested expand_short_bolt_id in gui.py:scan_pcba_barcode_window
        so headless input behaves identically to the GUI.
        """
        s = raw.strip()
        if not s:
            return s
    ...
    @staticmethod
    def _prompt_choice(prompt: str) -> int:
        """ENTER -> 1 (proceed), 's'/'skip' -> 2 (skip). Loops on bad input."""
        while True:
            ...
            if choice in ("s", "skip"):
                return 2
            print("Please press ENTER to proceed, or type 's' to skip.")

    # --- QR / barcode ----------------------------------------------------

    def scan_pcba_barcode_window(self) -> None:
        raw = input(
            "Scan the Bolt QR / PCB serial label (or type a short ID e.g. '181'), "
            "ENTER blank to skip: "
        ).strip()
        self._pcba_barcode = self._expand_short_bolt_id(raw) if raw else ""
    ...
    def update_test_indicator(self, test_number, passed) -> None:
        print(f"[STEP {test_number}] {'PASS' if passed else 'FAIL'}")

    # --- operator gates --------------------------------------------------

    def imu_instruction_window(self) -> None:
        print("\n--- IMU TEST ---")
        print("Rotate the board through its range of motion to trigger the IMU.")
        input("Press ENTER when you are ready to rotate the board... ")

    def sleep_current_window(self) -> int:
        print("\n--- PRODUCTION FLASH + SLEEP CURRENT ---")
        return self._prompt_choice(
            "About to flash production firmware and run the sleep current test. "
            "Press ENTER to proceed, or 's' to SKIP both: "
        )

    def ready_for_sleep_current_window(self) -> int:
        return self._prompt_choice(
            "Production firmware flashed. Ready the board, then press ENTER to "
            "measure sleep current, or 's' to SKIP the measurement: "
        )

    def ble_retry_window(self) -> None:
        print("\n*** BLE TEST FIRST FAILURE ***")
        print("This is likely a transient failure (re-power / advertising name change).")
        print("Please restart the test for the SAME board.")
        input("Press ENTER to acknowledge... ")

    def ppk2_lost_window(self) -> None:
        print("\n*** PPK2 LOST FROM USB ***")
        print("The PPK2 dropped off the USB bus and cannot be recovered in software.")
        print("Close this app, replug the PPK2 (check the cable/power switch), then reopen the fixture.")
        input("Press ENTER to acknowledge... ")

    def ppk2_press_button_window(self) -> None:
        print("\n*** PPK2 NOT RESPONDING ***")
        print("The PPK2 is stuck and a software restart did not recover it.")
        print("This is a fixture issue, not a board failure.")
        print("  1. Press the button on the PPK2 (next to the write/VOUT line) to power-cycle it.")
        print("  2. Wait for the PPK2 LED to come back on.")
        input("  3. Press ENTER — the fixture will re-grab the PPK2 and retry... ")

    def restart_fixture_window(self) -> None:
        print("\n*** ABNORMAL PPK2 READING DETECTED ***")
        print("This is a fixture issue, not a board failure. Please restart the fixture application.")
        input("Press ENTER to exit... ")

    def reboot_pi_window(self) -> bool:
        print("\n*** REPEATED PPK2 ABNORMAL READINGS ***")
        choice = input("Reboot the Raspberry Pi now? [y/N]: ").strip().lower()
        return choice == "y"

    def test_complete_window(self) -> None:
        print("\n=== TEST COMPLETE — open the lid ===")
        input("Press ENTER to continue to the next board... ")

    def information_window(self) -> None:
        print("\n" + "=" * 70)
        print("BOLT PCBA FIXTURE SETUP")
        print("1. Ensure the scanner and label printer USB cables are plugged in.")
        print("2. Power on the label printer and ensure it is ready.")
        print("3. Place the Bolt PCBA in the fixture.")
        print("=" * 70)
        input("Press ENTER when ready... ")

    def destroy(self) -> None:
        # Headless analogue of Tk's App.destroy(): request a clean exit from the
        # main loop (used on the PPK2 reboot path).
        self.should_exit = True


def prompt_for_bolt_qr(ui: ConsoleUI) -> str:
    """Capture the Bolt QR string via the console barcode prompt."""
    ui.scan_pcba_barcode_window()
    qr_payload = ui.get_pcba_barcode()
...
def gate(step_label: str) -> bool:
    ...
    Returns True to run the step, False to skip it. Skipping does not abort the
    run — the caller records the step as skipped and proceeds to the next one.
    """
    while True:
        choice = input(f"\n>>> Next: {step_label}. Press ENTER to RUN, or 's' to SKIP: ").strip().lower()
        if choice == "":
            return True
        if choice in ("s", "skip"):
            return False
        print("Please press ENTER to run, or type 's' to skip.")


def record_skip(test: BoltTest, ui: ConsoleUI, key: str, indicator: int, reason: str) -> None:
    """Record an operator-skipped test step.

    Uses a truthy sentinel so evaluate_overall_result() does not fail the board,
    logs the skip via upload_results.mark_skipped(), and marks the indicator green.
    """
    print(f"Step {indicator}: {key} - SKIPPED by operator")
    test.tests[key] = SG_SKIPPED_RESULT
    try:
        upload_results.mark_skipped(reason)
    except Exception as exc:
        print(f"mark_skipped failed (non-fatal): {exc}")
    ui.update_test_indicator(indicator, True)


def run_bolt_test(ui: ConsoleUI, skip_cal: bool = False, sg: bool = False) -> BoltTest:
    test = BoltTest()
    start_time = time.time()
    ...
    try:
        ...
        if not gate("Step 1: Scan Bolt QR"):
            record_skip(test, ui, "qr_scan", 1, "QR scan skipped by operator")
        else:
            qr_payload = prompt_for_bolt_qr(ui)
            if not qr_payload:
                print("QR scan: no data received from scanner; aborting test.")
                ui.update_test_indicator(1, False)
                test.failure = True
                return test
            ...
            ui.update_test_indicator(1, True)

        # Step 2: flash test firmware.
        if not gate("Step 2: Flash test firmware"):
            record_skip(test, ui, "flash_test_fw", 2, "flash test firmware skipped by operator")
        else:
            if not test.flash_test_firmware():
                ui.update_test_indicator(2, False)
                return test
            ui.update_test_indicator(2, True)
        ...
        if not gate("Step 3: USB connection"):
            record_skip(test, ui, "usb_connection", 3, "USB connection skipped by operator")
        else:
            if not test.open_serial_port():
                ui.update_test_indicator(3, False)
                return test
            ui.update_test_indicator(3, True)
            time.sleep(8.0)

        # Step 4: set serial on DUT.
        if not gate("Step 4: Set serial on DUT"):
            record_skip(test, ui, "set_serial", 4, "set serial skipped by operator")
        else:
            if not test.program_serial_on_dut():
                ui.update_test_indicator(4, False)
                return test
            ui.update_test_indicator(4, True)

        # Step 5: IMU test (manual rotation).
        if sg:
            print("IMU test: --SG set; skipping IMU test and recording result as 'sg'.")
            test.tests["imu"] = SG_SKIPPED_RESULT
            ui.update_test_indicator(5, True)
        elif not gate("Step 5: IMU test"):
            record_skip(test, ui, "imu", 5, "IMU test skipped by operator")
        else:
            ui.imu_instruction_window()
            if not test.run_imu_test():
                ui.update_test_indicator(5, False)
                return test
            ui.update_test_indicator(5, True)

        # Step 6: BLE test.
        if not gate("Step 6: BLE test"):
            record_skip(test, ui, "ble", 6, "BLE test skipped by operator")
        else:
            ...
            if not ble_ok:
                ...
                ui.update_test_indicator(6, False)
                if test.ble_first_failure:
                    ...
                    return test
            ui.update_test_indicator(6, True)
        ...
        if sg:
            ...
            ui.update_test_indicator(7, True)
        elif skip_cal:
            ...
            ui.update_test_indicator(7, True)
        elif not gate("Step 7: Analog calibration"):
            record_skip(test, ui, "analog", 7, "analog calibration skipped by operator")
        elif not test.run_analog_calibration():
            ui.update_test_indicator(7, False)
            return test
        else:
            ui.update_test_indicator(7, True)

        # Step 8 + 9: gate — production flash + sleep current (or skip both)
        if sg:
            ...
            if not test.flash_production_firmware(sg=True):
                ui.update_test_indicator(8, False)
                return test
            ui.update_test_indicator(8, True)
            ...
            ui.update_test_indicator(9, True)
            sleep_test_result = True
        elif (sleep_current_choice := ui.sleep_current_window()) == 2:
            ...
            ready_choice = ui.ready_for_sleep_current_window()
            if ready_choice == 2:
                ...
                try:
                    ...
                    for attempt in range(1, max_attempts + 1):
                        ...
                        # Prefer a no-touch software restart when the PPK2 is
                        # still on the bus and wasn't flagged lost.
                        software_ok = False
                        if ppk2.device_present() and not test.ppk2_lost:
                            print(f"Sleep current: attempt {attempt}/{max_attempts} failed; restarting PPK2 and retrying")
                            software_ok = ppk2.restart_ppk2()
                            if not software_ok:
                                print("Sleep current: software PPK2 restart failed")

                        if not software_ok:
                            # Hardware-stuck PPK2: ask the operator to press the
                            # button on the PPK2 to power-cycle it, then re-grab
                            # the handle. Retry a couple of times before giving up.
                            recovered = False
                            for btn_try in range(1, 3):
                                print(f"Sleep current: PPK2 stuck - requesting operator power-cycle (try {btn_try}/2)")
                                ui.ppk2_press_button_window()
                                if ppk2.reacquire_ppk2():
                                    recovered = True
                                    break
                                print("Sleep current: PPK2 still not detected after button press")
                            if not recovered:
                                test.ppk2_lost = True
        ...
        if test.ppk2_lost and sleep_current_choice != 2:
            ui.update_test_indicator(9, False)
            ...
            if error_count == 1:
                ...
                if ui.reboot_pi_window():
                    ...
                    try:
                        ...
                        # Give a moment for the command to be processed, then exit
                        time.sleep(0.5)
                        ui.destroy()
                    except subprocess.TimeoutExpired:
                        # Command was sent, continue with exit
        ...
        ui.update_test_indicator(10, final_ok)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bolt PCBA test fixture runner (headless)")
    parser.add_argument(
        "--SKIP_CAL",
        action="store_true",
    ...
    while True:
        bolt_test = run_bolt_test(ui, skip_cal=skip_cal, sg=sg)

        # PPK2 reboot path requested a clean exit (ui.destroy()).
        if ui.should_exit:
            print("Exiting fixture (reboot/exit requested).")
            break
        ...
        if getattr(bolt_test, "ble_first_failure", False):
            ...
            # Restart loop without test_complete prompt, label, CSV, or power-off
            continue

        # End‑of‑test prompt.
        ui.test_complete_window()
